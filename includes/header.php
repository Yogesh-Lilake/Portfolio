<?php
require_once dirname(__DIR__) . "/core/HeaderData.php";
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
    ...
    <a href="<?= HOME_URL ?>" class="flex items-center space-x-2 group">
      <img src="<?= htmlspecialchars($header['logo_path']) ?>"
           class="h-9 w-9 rounded-full transition-transform group-hover:rotate-12 transition-transform duration-300 shadow-md shadow-[<?= htmlspecialchars($header['accent_color']) ?>55]">
      <span class="logo-text gradient-text tracking-wide transition-all duration-300"><?= htmlspecialchars($header['site_title']) ?></span>
    </a>
    ...
    <!-- Mobile Menu -->
    <button id="menuBtn" class="md:hidden flex items-center p-2 rounded hover:text-accent transition-transform duration-300 hover:rotate-90">
